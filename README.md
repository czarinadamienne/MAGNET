...
One of the main observation is that the CUDA implementation is slower than the C, which could be due to a small dataset size and CUDA works more efficiently with larger datasets. This is possibly due to the data transfer overhead from CUDA taking up time while the C implementation is almost finished completing the program run. The CPU experiences less overhead allowing for it execute faster since the dataset size used was small in value. The CUDA implementation's speed up would be more prevalent at higher dataset sizes as there are more data elements to be processed which can utilize the GPU's full parallel processing capabilities. However, the current CUDA implementation causes a segmentation fault at larger dataset sizes.
